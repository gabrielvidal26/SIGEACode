...
<?php 
    ...
    $sqlcode = "SELECT * FROM evento WHERE nome LIKE '%$busca%' LIMIT $exibe,$itens_por_pagina "; 
    $execute = $mysqli->query($sqlcode) or die($mysqli->error);
    $evento = $execute->fetch_assoc();
    $num = $execute->num_rows;

    //Pega a quantidade total de objetos no banco de dados
    $num_total = $mysqli->query("SELECT id,nome,descricao FROM evento WHERE nome LIKE '%$busca%'")->num_rows;
                ...
                <div class="ofertas">    
                    <div class="row">
                        ...
                        <nav>
                          <ul class="pagination">
                            <li>
                              <a href="pesquisar.php?pagina=1&pesquisar=<?php echo $busca ?>" aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                              </a>
                            </li>
                            <?php for($i=0;$i<$num_paginas;$i++){ ?>
                              <li> <a href="pesquisar.php?pagina=<?php echo $i+1;?>&pesquisar=<?php echo $busca ?>"><?php echo $i+1; ?></a> </li>
                            <?php } ?>
                            <li>
                              <a href="pesquisar.php?pagina=<?php echo $num_paginas;?>&pesquisar=<?php echo $busca ?>" aria-label="Next">
